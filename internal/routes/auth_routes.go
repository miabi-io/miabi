...
func (r *Router) authRoutes() []okapi.RouteDefinition {
	...
	return []okapi.RouteDefinition{
		...
		},
		{
			...
			Group:       auth,
			Middlewares: limited,
			Handler:     okapi.H(r.h.auth.LoginToken),
			Summary:     "Re-authenticate and mint a short-lived CLI API token",
			Request:     &handlers.LoginTokenRequest{},
			Response:    &dto.Response[handlers.LoginTokenResponse]{},
		},
		{
			Method:      http.MethodPost,
			Path:        "/login-token/claim",
			Group:       auth,
			Middlewares: limited,
			Handler:     okapi.H(r.h.auth.ClaimLoginToken),
			Summary:     "Claim a login token minted by the SSO flow (single-use hand-off)",
			Request:     &handlers.ClaimLoginTokenRequest{},
			Response:    &dto.Response[handlers.LoginTokenResponse]{},
		},
		{
			Method:      http.MethodPost,
