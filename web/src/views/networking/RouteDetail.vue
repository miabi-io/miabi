...
const tabs = [
  { key: 'overview', label: 'routes.overview' },
  { key: 'settings', label: 'routes.settings' },
] as const
...
async function saveChain() {
  ...
  try {
    ...
    notify.success(t('notify.routeDetail.orderSaved'))
  } catch (e) {
    notify.apiError(e)
  } finally {
    mwBusy.value = false
...
async function copy(text: string) {
  try {
    ...
    notify.success(t('notify.common.copiedToClipboard'))
  } catch {
...
async function attachMiddleware() {
  ...
  try {
    ...
    syncChain(item.value)
    selectedMw.value = ''
    notify.success(t('notify.routeDetail.attached', { name }))
  } catch (e) {
    notify.apiError(e)
  } finally {
    mwBusy.value = false
...
async function detachMiddleware(name: string) {
  ...
  try {
    ...
    syncChain(item.value)
    notify.success(t('notify.routeDetail.detached', { name }))
  } catch (e) {
    notify.apiError(e)
  } finally {
    mwBusy.value = false
...
async function toggleEnabled() {
  ...
  try {
    ...
    notify.success(t(r.enabled ? 'notify.routeDetail.disabled' : 'notify.routeDetail.enabled'))
    await load()
  } catch (e) {
    notify.apiError(e)
  }
...
async function setSecurity(body: { exploit_protection?: boolean }) {
  ...
  try {
    ...
    notify.success(t('notify.routeDetail.securityUpdated'))
    await load(true)
  } catch (e) {
    notify.apiError(e)
  } finally {
...
async function saveMaintenance(enabled: boolean) {
  ...
  try {
    ...
    notify.success(t(enabled ? 'notify.routeDetail.maintenanceOn' : 'notify.routeDetail.maintenanceOff'))
    await load(true)
  } catch (e) {
    notify.apiError(e)
  } finally {
...
async function removeRoute() {
  ...
  try {
    ...
    notify.success(t('notify.routeDetail.deleted'))
    router.replace('/routes')
  } catch (e) {
    notify.apiError(e)
  } finally {
  ...
  <div v-if="item">
    <div class="page-header">
      <div class="title-group">
        <button class="btn-icon btn-icon-muted" :title="$t('routes.back')" :aria-label="$t('routes.back')" @click="router.push('/routes')">
        ...
        <span v-if="item.generated" class="badge badge-info badge-dot" :title="$t('routes.generatedHint')">{{ $t('routes.autoGenerated') }}</span>
        <span v-if="maintenanceOn" class="badge badge-warning badge-dot" :title="$t('routes.maintenanceHint')">{{ $t('routes.maintenance') }}</span>
    ...
    <div v-if="tab === 'overview'">
      <div class="card mb-4">
        <div class="card-header"><h2>{{ $t('routes.configuration') }}</h2></div>
        <div class="card-body detail-list">
          <div class="detail-row"><span class="detail-key">{{ $t('routes.application') }}</span><span>{{ app?.name || `#${item.application_id}` }}</span></div>
          <div class="detail-row"><span class="detail-key">{{ $t('routes.path') }}</span><span class="mono">{{ item.path }}</span></div>
          <div class="detail-row">
            <span class="detail-key">{{ $t('routes.hosts') }}</span>
          ...
          <div class="detail-row"><span class="detail-key">{{ $t('routes.methods') }}</span><span>{{ (item.methods || []).join(', ') || 'all' }}</span></div>
          <div class="detail-row"><span class="detail-key">{{ $t('routes.targetPort') }}</span><span class="mono">{{ port }}</span></div>
          <div class="detail-row">
            <span class="detail-key">TLS</span>
            <span><span class="badge badge-neutral">{{ item.tls_mode }}</span><span v-if="item.has_custom_cert" class="text-muted text-sm" style="margin-left: 8px">{{ $t('routes.customCert') }}</span></span>
          </div>
        </div>
      </div>
      ...
      <div class="card mb-4">
        <div class="card-header"><h2>{{ $t('routes.middlewares') }}</h2></div>
        <div class="card-body">
          <p class="text-muted text-sm" style="margin-top: 0">{{ $t('routes.middlewaresHint') }}<template v-if="item.generated">{{ $t('routes.middlewaresOnGeneratedHint') }}</template>
          </p>
            ...
            <li v-for="(m, i) in chain" :key="m" class="mw-chain-item">
              ...
              <span v-if="ws.canEdit" class="mw-move">
                <button
                  ...
                  :disabled="mwBusy || i === 0"
                  :title="$t('routes.moveEarlier')"
                  :aria-label="$t('routes.moveEarlier')"
                  @click="moveMw(i, i - 1)"
                ><span class="mdi mdi-arrow-up"></span></button>
                <button
                  class="btn-icon btn-icon-muted"
                  :disabled="mwBusy || i === chain.length - 1"
                  :title="$t('routes.moveLater')"
                  :aria-label="$t('routes.moveLater')"
                  @click="moveMw(i, i + 1)"
                ><span class="mdi mdi-arrow-down"></span></button>
                <button
                  class="btn-icon btn-icon-danger"
                  :disabled="mwBusy || chainDirty"
                  :title="chainDirty ? $t('routes.saveOrderFirst') : $t('routes.detachMiddleware')"
                  :aria-label="$t('routes.detachMiddleware')"
                  @click="detachMiddleware(m)"
                ><span class="mdi mdi-close"></span></button>
              </span>
            </li>
          </ol>
          <p v-else class="text-muted text-sm mw-empty">{{ $t('routes.noMiddlewaresAttached') }}</p>

          <p v-if="chain.length > 1" class="text-muted text-sm mw-order-hint">
            <span class="mdi mdi-information-outline"></span>
            <i18n-t keypath="routes.chainHint" tag="span"><template #first><strong>{{ chain[0] }}</strong></template></i18n-t>
          </p>

          <div v-if="chainDirty" class="mw-chain-actions">
            <button class="btn btn-secondary btn-sm" :disabled="mwBusy" @click="syncChain(item)">{{ $t('routes.discard') }}</button>
          ...
          <div v-if="ws.canEdit" class="mw-attach">
            ...
            <button class="btn btn-secondary btn-sm" :disabled="!selectedMw || mwBusy" @click="attachMiddleware">
              ...
              <span v-else>{{ $t('routes.attach') }}</span>
            </button>
          </div>
          <i18n-t v-if="ws.canEdit && !allMiddlewares.length" keypath="routes.noMiddlewaresHint" tag="p" class="text-muted text-sm mw-empty-hint"><template #link><a href="#" @click.prevent="router.push('/middlewares')">{{ $t('routes.createOne') }}</a></template></i18n-t>
        </div>
      </div>
      ...
      <div v-if="hosts.length" class="card mb-4">
        ...
        <div class="card-body">
          <p class="text-muted text-sm" style="margin-top: 0">{{ $t('routes.dnsHint') }}</p>
          <div v-if="dnsTarget || dnsHostname" class="dns-table">
            <div class="dns-head">
              <span>{{ $t('routes.type') }}</span><span>{{ $t('apps.form.name') }}</span><span>{{ $t('routes.value') }}</span><span></span>
            </div>
            <div v-for="h in hosts" :key="h" class="dns-row">
              ...
              <button class="btn-icon btn-icon-muted" :title="$t('routes.copyValue')" :aria-label="$t('routes.copyValue')" @click="copy(dnsTarget || dnsHostname)"><span class="mdi mdi-content-copy"></span></button>
            </div>
          </div>
          <div v-else class="dns-empty">
            <span class="mdi mdi-alert-outline"></span>
            <i18n-t keypath="routes.noPublicAddressHint" tag="span"><template #field><strong>{{ $t('routes.publicIp') }}</strong></template></i18n-t>
          </div>
        </div>
      </div>

      <div class="card">
        <div class="card-header"><h2>{{ $t('routes.backends') }}</h2></div>
        <div class="card-body">
          <div class="backend-row">
            <span class="badge badge-success badge-dot">{{ $t('routes.stable') }}</span>
          ...
          <div v-if="canaryActive" class="backend-row">
            <span class="badge badge-warning badge-dot">{{ $t('routes.canary') }}</span>
          ...
          <i18n-t v-if="canaryActive" keypath="routes.canaryHint" tag="p" class="text-muted text-sm mt-4"><template #tab><strong>{{ $t('routes.deployments') }}</strong></template></i18n-t>
          <p v-else class="text-muted text-sm mt-4">{{ $t('routes.backendsHint') }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-else>
      <div class="card mb-4">
        <div class="card-header"><h2>{{ $t('routes.maintenanceTitle') }}</h2></div>
        <div class="card-body detail-list">
          <div class="mt-state" :class="maintenanceOn ? 'is-parked' : 'is-live'">
            ...
            <div class="mt-state-copy">
              ...
              <div class="text-muted text-sm">
                <i18n-t v-if="maintenanceOn" keypath="routes.maintenanceOnHint" tag="span"><template #status><strong>{{ effectiveStatus }}</strong></template></i18n-t>
                <template v-else>{{ $t('routes.maintenanceOffHint') }}</template>
              </div>
            </div>
            <button
              class="btn btn-sm"
          ...
          <div class="mt-response">
            <div class="mt-response-head">
              <div class="cell-title">{{ $t('routes.responseWhileParked') }}</div>
              <div class="text-muted text-sm">
            ...
            <div class="mt-fields">
              <label class="mt-field mt-field-status">
                <span class="form-label">{{ $t('routes.statusCode') }}</span>
                <input
                  ...
                  :disabled="!ws.canEdit"
                />
                <span v-if="statusInvalid" class="form-error">{{ $t('routes.statusCodeHint') }}</span>
                <span v-else class="form-hint">{{ $t('routes.4xxOr5xx') }}</span>
              </label>

              <label class="mt-field mt-field-message">
                <span class="form-label">{{ $t('routes.message') }} <span v-if="messageIsJson && mtMessage.trim()" class="badge badge-info mt-json-badge">{{ $t('routes.validJson') }}</span>
                </span>
                <textarea
                  v-model="mtMessage"
                  ...
                  maxlength="1024"
                  spellcheck="false"
                  :placeholder="$t('routes.backAt1400Utc')"
                  :disabled="!ws.canEdit"
                ></textarea>
            ...
            <div class="mt-preview">
              <div class="mt-preview-head">
                <span class="form-label" style="margin: 0">{{ $t('routes.whatTheCallerGets') }}</span>
...
{{ previewBody }}</code></pre>
              <p class="text-muted text-sm mt-preview-note">
                <i18n-t v-if="previewAs === 'text'" keypath="routes.previewTextHint" tag="span"><template #header><code>Accept</code></template></i18n-t>
                <template v-else-if="previewAs === 'json'">
                  <template v-if="messageIsJson">{{ $t('routes.messageIsJson') }}</template>
                  <template v-else>{{ $t('routes.messageNotJson') }}</template>
                </template>
                <template v-else>{{ $t('routes.messageXmlHint') }}</template>
              </p>
            </div>
      ...
      <div class="card mb-4">
        <div class="card-header"><h2>{{ $t('routes.gatewaySecurity') }}</h2></div>
        <div class="card-body detail-list">
          <div class="detail-row">
            <span>
              <div class="cell-title">{{ $t('routes.exploitProtection') }}</div>
              <div class="text-muted text-sm">{{ $t('routes.exploitProtectionHint') }}</div>
            </span>
            <button
              class="btn btn-secondary btn-sm"
      ...
      <div v-if="item.generated" class="card">
        <div class="card-header"><h2>{{ $t('routes.settings') }}</h2></div>
        <div class="card-body">
          <div class="managed-note">
            <span class="mdi mdi-lock-outline"></span>
            <div>
              <div class="cell-title">{{ $t('routes.autoGeneratedManaged') }}</div>
              <i18n-t keypath="routes.generatedRouteHint" tag="div" class="text-muted text-sm"><template #card><strong>{{ $t('routes.externalAccess') }}</strong></template></i18n-t>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-body detail-list">
          <div class="detail-row">
            <span>
              ...
              <div class="text-muted text-sm">{{ $t('routes.disableHint') }}</div>
            </span>
          ...
          <div class="detail-row">
            <span>
              <div class="cell-title">{{ $t('routes.editRoute') }}</div>
              <div class="text-muted text-sm">{{ $t('routes.editHint') }}</div>
            </span>
            <button class="btn btn-secondary btn-sm" @click="router.push('/routes')">{{ $t('routes.openEditor') }}</button>
          </div>
          <div class="detail-row">
            <span>
              <div class="cell-title" style="color: var(--danger-600)">{{ $t('confirm.title.deleteRoute') }}</div>
              <div class="text-muted text-sm">{{ $t('routes.deleteHint') }}</div>
            </span>
            <button class="btn btn-danger btn-sm" :disabled="!ws.canEdit" @click="showDelete = true">{{ $t('action.delete') }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <ConfirmDialog
      ...
      :message="$t('confirm.message.routeDetail.deleteRouteNameIts', { name: item?.name })"
