...
async function save() {
  ...
  try {
    if (editingId.value) {
      ...
      notify.success(t('notify.ldapDirectory.updated'))
    } else {
      await adminApi.createLdap(form.value)
      notify.success(t('notify.ldapDirectory.created'))
    }
...
async function test(c: LdapConfig) {
  ...
  try {
    ...
    if (res.ok) notify.success(res.message || t('notify.ldapDirectory.connectionOk'))
    else notify.error(res.error || t('notify.ldapDirectory.connectionFailed'), { title: t('notify.ldapDirectory.testFailed') })
  } catch (e) {
    notify.apiError(e)
  } finally {
...
async function confirmDelete() {
  ...
  try {
    ...
    notify.success(t('notify.ldapDirectory.deleted'))
    toDelete.value = null
    await load()
  } catch (e) {
    notify.apiError(e)
...
async function addMapping() {
  ...
  try {
    ...
    notify.success(t('notify.ldapDirectory.mappingAdded'))
...
async function removeMapping(m: LdapGroupMapping) {
  ...
  try {
    ...
    notify.success(t('notify.ldapDirectory.mappingRemoved'))
    await refreshMappingConfig()
  } catch (e) {
    notify.apiError(e)
  }
...
<template>
  <div>
    <div class="page-header">
      <div>
        <h1>{{ $t('ldap.ldapActiveDirectory') }}</h1>
        <p class="subtitle">{{ $t('ldap.upsell') }}</p>
      </div>
      <button v-if="ldap.has.value" class="btn btn-primary" :disabled="!canWrite" :title="canWrite ? '' : 'License is read-only'" @click="openCreate">
        <span class="mdi mdi-plus"></span>{{ $t('ldap.addConnection') }}</button>
    </div>
    ...
    <div v-if="!ldap.has.value" class="card">
      <div class="empty-state">
        ...
        <h3>{{ $t('ldap.enterpriseFeature') }}</h3>
        <p :title="lockTitle">{{ $t('ldap.subtitle') }}</p>
        <router-link to="/admin/license" class="btn btn-secondary mt-4">{{ $t('plans.manageLicense') }}</router-link>
      </div>
    </div>
    ...
    <template v-else>
      <div class="card">
        ...
        <div v-else-if="configs.length === 0" class="empty-state">
          ...
          <h3>{{ $t('ldap.noDirectoriesConnected') }}</h3>
          <p>{{ $t('ldap.emptyHint') }}</p>
          <button class="btn btn-primary mt-4" :disabled="!canWrite" @click="openCreate">{{ $t('ldap.addAConnection') }}</button>
        </div>
        <div v-else class="table-wrapper">
          <table>
            <thead><tr><th>{{ $t('apps.form.name') }}</th><th>{{ $t('ldap.server') }}</th><th>TLS</th><th>{{ $t('ldap.groups') }}</th><th>{{ $t('dashboard.col.status') }}</th><th></th></tr></thead>
            <tbody>
              <tr v-for="c in configs" :key="c.id">
                ...
                <td class="text-right table-actions">
                  <button class="btn-icon btn-icon-muted" :title="$t('ldap.testConnection')" :aria-label="$t('ldap.testConnection')" :disabled="testingId === c.id" @click="test(c)">
                    <span class="mdi" :class="testingId === c.id ? 'mdi-loading mdi-spin' : 'mdi-connection'"></span>
                  </button>
                  <button class="btn-icon btn-icon-muted" :title="$t('ldap.groupMappings')" :aria-label="$t('ldap.groupMappings')" @click="openMappings(c)"><span class="mdi mdi-account-multiple-outline"></span></button>
                  <button class="btn-icon btn-icon-muted" :title="$t('action.edit')" :aria-label="$t('action.edit')" :disabled="!canWrite" @click="openEdit(c)"><span class="mdi mdi-pencil-outline"></span></button>
                  <button class="btn-icon btn-icon-danger" :title="$t('action.delete')" :aria-label="$t('action.delete')" :disabled="!canWrite" @click="toDelete = c"><span class="mdi mdi-delete-outline"></span></button>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <form @submit.prevent="save">
          <div class="modal-body">
            <div class="form-group">
              ...
              <input v-model="form.display_name" class="form-input" :placeholder="$t('ldap.namePlaceholder')" required autofocus />
            </div>

            <h4 class="form-section">{{ $t('ldap.connection') }}</h4>
            <div class="form-row">
              <div class="form-group" style="flex: 2; margin-bottom: 0">
                <label class="form-label">{{ $t('ldap.host') }}</label>
                <input v-model="form.host" class="form-input mono" placeholder="ldap.corp.example.com" required />
              </div>
              <div class="form-group" style="flex: 1; margin-bottom: 0">
                <label class="form-label">{{ $t('ldap.port') }}</label>
            ...
            <div class="form-row" style="margin-top: 12px">
              <div class="form-group" style="flex: 1; margin-bottom: 0">
                ...
                <select v-model="form.tls_mode" class="form-select" :aria-label="$t('ldap.tlsMode')">
                  <option value="starttls">{{ $t('ldap.tlsStartTls') }}</option>
                  <option value="ldaps">{{ $t('ldap.tlsLdaps') }}</option>
                  <option value="none">{{ $t('ldap.tlsNone') }}</option>
                </select>
              </div>
              <div class="form-group" style="flex: 1; margin-bottom: 0">
            ...
            <div class="form-group" style="margin-top: 12px">
              <label class="form-label">{{ $t('ldap.caCert') }}</label>
            ...
            <label class="check-row">
              ...
              <span>{{ $t('ldap.skipTlsCertificateVerification') }}<span class="text-muted">{{ $t('ldap.insecureNote') }}</span></span>
            </label>

            <h4 class="form-section">{{ $t('ldap.serviceAccountBind') }}</h4>
            <div class="form-group">
              <label class="form-label">{{ $t('ldap.bindDn') }}</label>
              <input v-model="form.bind_dn" class="form-input mono" placeholder="cn=miabi,ou=svc,dc=corp,dc=example,dc=com" />
            </div>
            <div class="form-group">
              <label class="form-label">{{ $t('ldap.bindPassword') }}</label>
    ...
    <Teleport to="body">
      <AppModal v-if="mappingConfig" max-width="620px" @close="mappingConfig = null">
        <div class="modal-header">
          ...
          <button class="btn-icon btn-icon-muted" :aria-label="$t('shell.close')" @click="mappingConfig = null"><span class="mdi mdi-close"></span></button>
        </div>
        <div class="modal-body">
          <p class="text-muted text-sm">{{ $t('ldap.mappingHint') }}</p>
          <div v-if="(mappingConfig.mappings || []).length" class="table-wrapper" style="margin: 12px 0">
            <table>
              <thead><tr><th>{{ $t('ldap.group') }}</th><th>{{ $t('ldap.grants') }}</th><th></th></tr></thead>
              <tbody>
                <tr v-for="m in mappingConfig.mappings" :key="m.id">
                  <td class="cell-sub mono">{{ m.group_dn }}</td>
                  <td class="cell-sub">
                    <span v-if="m.system_admin" class="badge badge-warning">{{ $t('ldap.platformAdmin') }}</span>
                    <span v-if="m.workspace_id" class="badge badge-neutral">ws #{{ m.workspace_id }} · {{ m.workspace_role }}</span>
                  </td>
                  <td class="text-right">
                    <button class="btn-icon btn-icon-danger" :title="$t('ldap.removeMapping')" :aria-label="$t('ldap.removeMapping')" :disabled="!canWrite" @click="removeMapping(m)"><span class="mdi mdi-delete-outline"></span></button>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
          <p v-else class="text-muted text-sm" style="margin: 12px 0">{{ $t('ldap.noMappings') }}</p>

          <form v-if="canWrite" class="map-form" @submit.prevent="addMapping">
            <div class="form-group">
              <label class="form-label">{{ $t('ldap.groupDnOrCn') }}</label>
              <input v-model="mapForm.group_dn" class="form-input mono" :placeholder="$t('ldap.groupDnPlaceholder')" required />
            </div>
            <div class="form-row" style="align-items: flex-end">
              <label class="check-row" style="flex: 1; margin: 0">
                <input v-model="mapForm.system_admin" type="checkbox" />
                <span>{{ $t('nav.palette.adminGroup') }}</span>
              </label>
              <div class="form-group" style="flex: 1; margin-bottom: 0">
                <label class="form-label">{{ $t('oauth.workspaceId') }}</label>
                <input v-model.number="mapForm.workspace_id" type="number" class="form-input" :placeholder="$t('ldap.optional')" />
              </div>
              <div class="form-group" style="flex: 1; margin-bottom: 0">
                <label class="form-label">{{ $t('ldap.workspaceRole') }}</label>
                <select v-model="mapForm.workspace_role" class="form-select" :aria-label="$t('ldap.workspaceRole')" :disabled="!mapForm.workspace_id">
                  <option value="owner">{{ $t('ldap.owner') }}</option>
                  <option value="admin">{{ $t('ldap.admin') }}</option>
                  <option value="developer">{{ $t('ldap.developer') }}</option>
                  <option value="viewer">{{ $t('ldap.viewer') }}</option>
                </select>
              </div>
              <button type="submit" class="btn btn-primary" :disabled="savingMapping">{{ $t('ldap.add') }}</button>
            </div>
          </form>
        </div>
      </AppModal>
